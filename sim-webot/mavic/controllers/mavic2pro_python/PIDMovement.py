...
class MovementController:
    ...
    def __init__(self, kp, ki, kd, max_pitch=6.0, max_roll=4.0, distance_threshold=1.0):
        """
        Args:
            ...
            max_pitch: Maximum pitch disturbance (degrees or motor input)
            max_roll: Maximum roll disturbance (degrees or motor input)
    ...
    def update(self, delta_x, delta_y, velocity_x, velocity_y, accel_magnitude, dt):
        ...
        stopping_distance = max(stopping_distance, 0.5)
        stopping_distance *= 3.0

        speed_limit_factor = 1.0
        if distance < stopping_distance * 2.5:
            speed_limit_factor = max(0.15, distance / (stopping_distance * 2.5))
